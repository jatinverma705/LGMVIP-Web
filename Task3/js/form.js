function submitForm() {
    ...
    if (age == "") {
      ageInput = document.forms["regForm"]["age"]
      ageInput.style.border = "1px solid red";
      ageInput.className += "error"
    ...
    ageText.style.marginBottom = "5px"
    ...
    description.appendChild(ageText);

    const games = document.createElement("p")
    games.id = "games";
    const gameBtns = document.querySelectorAll('input[name="game"]');
    let count = 0;
    for (const gameBtn of gameBtns) {
      if (gameBtn.checked) {
        games.append(gameBtn.value + " ");
        count += 1
        gameBtn.checked = false;
      }
    } 
    if (count == 0){
      games.append("None")
    ...
    document.forms["regForm"]["age"].value = null;
